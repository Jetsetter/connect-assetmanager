...
module.exports = function assetManager (assets) {
	...
	self.fileQueue = async.queue(function(file, callback){
	...
	this.getFile = function (file, path, groupName, callback) {
		...
		if (isExternal) {
			...
			self.fileQueue.push(path+file, function(data){
				if(!data){
					console.log('Could not find: '+file);
					callback(null, '');
					return;
				...
				fileInfo.content = data.readFile.toString();
				fileInfo.modified = data.stat.mtime;
				callback(null, fileInfo);
			});
